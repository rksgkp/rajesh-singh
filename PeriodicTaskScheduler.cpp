...
void PeriodicScheduler::PeriodicTask(const std::uint32_t &id, std::function<void()> f, const std::chrono::system_clock::time_point &tp, const int &s)
{
        // Create Task object
        Task T(id,  f, tp, std::chrono::seconds(s));
        {
                ...
                auto now = std::chrono::system_clock::now();
                {
                        ...
                        if (!task_queue.empty() && task_queue.top().time <= now)
                        {
                                ...
                                if (delete_task_set.find(task.uid) == delete_task_set.end())
                                {
                                        ...
                                        {
                                                PeriodicTask(task.uid, task.RunPeriodicTask, std::chrono::system_clock::now() + task.interval, task.interval.count());
                                        }
                                        else
                                        {
                                                // Schedule task with the new interval
                                                PeriodicTask(task.uid, task.RunPeriodicTask, std::chrono::system_clock::now() + search->second, search->second.count());
                                                std::unique_lock<std::mutex> lock(queue_mutex);
                                        ...
                                        // Execute the task
                                        std::cout << "Task id: "<< task.uid << ", Interval: " << task.interval.count() << std::endl;
                                        task.RunPeriodicTask();
                                }
                                else
                                {
...
void PeriodicScheduler::get_tasks_overview()
{
        ...
        //Display Task List
        std::cout << "+------" << "+--------------" << std::endl;
        std::cout << "|" << std::setw(5) << "UID" << " |" << std::setw(12) << "Interval"  << " |" << std::endl;
        std::cout << "+------" << "+--------------" << std::endl;
        while (!temp.empty()) {
                std::cout << "|" << std::setw(5) << temp.top().uid << " |" << std::setw(12) << temp.top().interval.count()  << " |" << std::endl;
                std::cout << "+------" << "+--------------" << std::endl;
                temp.pop();
        }
