...
struct Task
{
        boost::function<void()> RunPeriodicTask;
        ...
        */
        Task(const std::uint32_t id, std::function<void()> f, const std::chrono::system_clock::time_point tp, const std::chrono::seconds s)
                :RunPeriodicTask(f),
                interval(s),
                time(tp),
                uid(id)
...
public:
        ...
        */
        void PeriodicTask(const std::uint32_t &id, std::function<void()> f, const std::chrono::system_clock::time_point &tp, const int &s);
